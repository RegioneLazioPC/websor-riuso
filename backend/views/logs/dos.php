...
$dataProvider = $filterModel->search( Yii::$app->request->get(), 'common\models\RichiestaDos');
...
$cols = [
            ...
            [
                ...
                'value' => function($model, $index, $widget){
                    ...
                    switch ($model->action){
                        case 'CHANGE':
                            $action = 'Richiesta Dos modificata';
                            break;
                        case 'CREATE':
                            $action = 'Nuova richiesta Dos creata';
                            break;
                        case 'DELETE':
                            $action = 'Richiesta Dos cancellata';
                            break;
                        default:
...
echo GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'label' => 'N.Protocollo',
            'value' => function($model, $index, $widget){
                $ingaggio = RichiestaDos::findOne($model->model_id);
                if(!empty($ingaggio)) :
        ...
        [
            ...
            'filter'=> Html::activeDropDownList($filterModel, 'action', [
                    'CHANGE' => 'Richiesta Dos modificata',
                    'CREATE' => 'Nuova richiesta Dos creata',
                    'SET' => 'Modifica campo',
                    'DELETE' => 'Richiesta Dos cancellata',
        ...
        [
            'label' => 'Campo modificato',
            'attribute' => 'field',
            'filter'=> Html::activeDropDownList($filterModel, 'field', (new RichiestaDos)->attributeLabels(), ['class' => 'form-control','prompt' => 'Tutti']),
            'value' => function($model, $index, $widget){
                return $model->getParent()->getAttributeLabel($model->field);
            }
        ],
