...
$dataProvider = $filterModel->search( Yii::$app->request->get(), 'common\models\UtlIngaggio');
...
$cols = [
            ...
            [
                ...
                'value' => function($model, $index, $widget){
                    ...
                    switch ($model->action){
                        case 'CHANGE':
                            $action = 'Ingaggio modificato';
                            break;
                        case 'CREATE':
                            $action = 'Nuovo ingaggio creato';
                            break;
                        case 'DELETE':
                            $action = 'Ingaggio cancellato';
                            break;
                        default:
...
echo GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'label' => 'N.Protocollo',
            'value' => function($model, $index, $widget){
                $ingaggio = UtlIngaggio::findOne($model->model_id);
                if(!empty($ingaggio)) :
        ...
        [
            ...
            'filter'=> Html::activeDropDownList($filterModel, 'action', [
                'CHANGE' => 'Ingaggio modificato',
                'CREATE' => 'Nuovo ingaggio creato',
                'SET' => 'Modifica campo',
                'DELETE' => 'Ingaggio cancellato',
        ...
        [
            'label' => 'Campo modificato',
            'attribute' => 'field',
            'filter'=> Html::activeDropDownList($filterModel, 'field', (new UtlIngaggio)->attributeLabels(), ['class' => 'form-control','prompt' => 'Tutti']),
            'value' => function($model, $index, $widget){
                return $model->getParent()->getAttributeLabel($model->field);
            }
        ],
