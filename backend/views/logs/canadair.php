...
$dataProvider = $filterModel->search( Yii::$app->request->get(), 'common\models\RichiestaCanadair');
...
$cols = [
            ...
            [
                ...
                'value' => function($model, $index, $widget){
                    ...
                    switch ($model->action){
                        case 'CHANGE':
                            $action = 'Richiesta canadair modificata';
                            break;
                        case 'CREATE':
                            $action = 'Nuova richiesta canadair creata';
                            break;
                        case 'DELETE':
                            $action = 'Richiesta canadair cancellato';
                            break;
                        default:
...
echo GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'label' => 'N.Protocollo',
            'value' => function($model, $index, $widget){
                $ingaggio = RichiestaCanadair::findOne($model->model_id);
                if(!empty($ingaggio)) :
        ...
        [
            ...
            'filter'=> Html::activeDropDownList($filterModel, 'action', [
                'CHANGE' => 'Richiesta canadair modificata',
                'CREATE' => 'Nuova richiesta canadair creata',
                'SET' => 'Modifica campo',
                'DELETE' => 'Richiesta canadair cancellata',
        ...
        [
            'label' => 'Campo modificato',
            'attribute' => 'field',
            'filter'=> Html::activeDropDownList($filterModel, 'field', (new RichiestaCanadair)->attributeLabels(), ['class' => 'form-control','prompt' => 'Tutti']),
            'value' => function($model, $index, $widget){
                return $model->getParent()->getAttributeLabel($model->field);
            }
        ],
