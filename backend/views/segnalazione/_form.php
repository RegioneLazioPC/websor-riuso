...
function generate_checkbox_code($checkbox, $model) {
    ...
    if($checkbox->show_numero){
        $detail .= "N. <input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero]' value='{$detailExtra['numero']}' />";
    }

    //ShowNote
    if($checkbox->show_note){
        $detail .= "&nbsp;&nbsp;&nbsp;Note <input style='width:350px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][note]' value='{$detailExtra['note']}' />";
    }

    //ShowNumNucleiFamiliari
    if($checkbox->show_num_nuclei_familiari){
        $detail .= "&nbsp;&nbsp; di cui N. nuclei familiari <input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_nuclei_familiari]' value='{$detailExtra['numero_nuclei_familiari']}' />";
    }

    //show_num_disabili
    if($checkbox->show_num_disabili){
        $detail .= "&nbsp;&nbsp; N. disabili/persone non autosufficienti<input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_disabili]' value='{$detailExtra['numero_disabili']}' />";
    }

    //show_num_sistemazione_parenti_amici
    if($checkbox->show_num_sistemazione_parenti_amici){
        $detail .= "N. presso parenti/amici<input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_sistemazione_parenti_amici]' value='{$detailExtra['numero_sistemazione_parenti_amici']}' />";
    }

    //show_num_sistemazione_strutture_ricettive
    if($checkbox->show_num_sistemazione_strutture_ricettive){
        $detail .= "&nbsp;&nbsp; N. presso str.ricettive<input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_sistemazione_strutture_ricettive]' value='{$detailExtra['numero_sistemazione_strutture_ricettive']}' />";
    }

    //show_num_sistemazione_area_ricovero
    if($checkbox->show_num_sistemazione_area_ricovero){
        $detail .= "&nbsp;&nbsp; N. in aree di ricovero <input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_sistemazione_area_ricovero]' value='{$detailExtra['numero_sistemazione_area_ricovero']}' />";
    }

    //show_num_persone_isolate
    if($checkbox->show_num_persone_isolate){
        $detail .= "&nbsp;&nbsp; N. persone isolate <input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_persone_isolate]' value='{$detailExtra['numero_persone_isolate']}' />";
    }

    //show_num_utenze
    if($checkbox->show_num_utenze){
        $detail .= "&nbsp;&nbsp; n. utenze<input style='width:40px' type='text' name='UtlSegnalazioneExtraInfo[{$checkbox->id}][numero_utenze]' value='{$detailExtra['numero_utenze']}' />";
    }
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 p10h" ng-controller="AutocompleteController as ctrl" ng-init="init('<?php echo Yii::$app->request->csrfToken;?>', segCtrl)">
                    ...
                    <div class="toponimo_add" <?php if($model->address_type != 4) echo 'style="display: none;"';?> >
                        ...
                        <div class="col-lg-12 no-pl field-id_address">
                            ...
                            class="form-control" 
                            name="UtlSegnalazione[toponimo_address]">
                            <table class="auto_table" role="none">
                    ...
                    <?php echo $form->field($model, 'attachment',['options' => ['class'=>'col-lg-12 no-pl no-pr']])->widget(FileInput::classname(), [
                    ])->label('Allega file (audio telefonata, email)'); ?>
        ...
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            ...
            <?php else: ?>
                ...
                <div id="map-canvas-mod" class="site-index" ng-init="segCtrl.setLatLon(<?php echo $center_lat; ?>, <?php echo $center_lon; ?>)">
                    ...
                    >
                        <ui-gmap-marker coords="{latitude: segCtrl.lat, longitude: segCtrl.lon}" 
