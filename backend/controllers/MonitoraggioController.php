...
class MonitoraggioController extends Controller
{
    ...
    public function actionResend() {
        ...
        switch(Yii::$app->request->post('action')) {
            ...
            ->andWhere('(SELECT COUNT(id) FROM mas_single_send WHERE 
                ...
                ( (mas_single_send.channel = \'Pec\' OR mas_single_send.channel = \'Fax\' OR mas_single_send.channel = \'Sms\') AND mas_single_send.status = 3 ) OR 
                ( (mas_single_send.channel != \'Pec\' AND mas_single_send.channel != \'Fax\' AND mas_single_send.channel != \'Sms\') AND (mas_single_send.status = 3 OR mas_single_send.status = 2) )
            ) = 0');
            //->all();
            break;
            case 'selected':
    ...
    public function actionContacts( ) {
        Yii::$app->response->format = Response::FORMAT_JSON;
        return [ 'data' => \common\models\ConMasInvioContact::find()
    ...
    private function ciclateGroupedContacts( $list ) {
        foreach ($list as $contact) {
            try{
                ...
                foreach ($channels as $ch) {
                    ...
                    $ch_sent = array_filter($contact['masSingleSendsAggregated'], function($el) use ($ch, $const_mas_status){
                        
                        return $el['channel'] == $ch && in_array( $el['status'], $const_mas_status );
                    });             
                    $to_return['sent_'.$ch] = ( count($ch_sent) > 0 ) ? 'Si' : 'No';
                    if(count($ch_sent) > 0) $to_return['raggiunto'] = 'Si';
                }
                yield $to_return;
            } catch(\Exception $e) {                
    ...
    public function actionGroupedContacts( ) {
        ...
        $return_data = [];
        foreach ($this->ciclateGroupedContacts($list) as $result) {
            $return_data[] = $result;
        }
