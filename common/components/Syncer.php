...
class Syncer {
    ...
    {     
        try {
            ...
            foreach ($array as $contatto) {
                if($contatto['type'] == $type && $contatto['use_type'] == $use_type) $arr[] = $contatto['contatto'];                
            }
    ...
    private function getVolSedeTipo ( $tipo ) {
        switch($tipo){
            case 0: return 'Sede Legale'; break;
            default: return 'Sede Operativa'; break;
        }
    }
    ...
    private function getSede( $sede_array, $id_organizzazione ) 
    {
        ...
        if(!$sede) {
             ...
             */
            if(count($sedi_org) > 1 || count($sedi_org) == 0) return false;
    ...
    {
        if(!empty($ref_id)) $ref_id = intval($ref_id);
        ...
        if(!$ris) {
            ...
            if(!empty($ref_id)) $ris->orWhere(['ref_id'=>intval($ref_id)]);
            $ris = $ris->one();
        }
            ...
            foreach ($array as $k => $cont) {
                ...
                if(!empty($contatto->id_sync) && $contatto->id_sync == "MGO_".$cont['id']) {
                    ...
                    if(!$contatto->contatto->save()) {
                        ...
                        if(!empty($cont['note'])) $contatto->note = $cont['note'];
                        if(!empty($cont['type'])) $contatto->type = $cont['type'];
                        if ( !$contatto->save() ) {
    ...
    {
        if( preg_match("/^RAND_/", $data['cf']) ) return false;
        ...
        try {
            ...
            if(!$ana) $ana = new \common\models\UtlAnagrafica;
            ...
            if(!$ana->save()) {
                ...
                if(!empty($data['contatto'])) $this->syncContatti( $ana, $data['conAnagraficaContatto'] );
                if(!empty($data['indirizzo'])) $this->syncIndirizzi( $ana, $data['indirizzo'] );
    ...
    private function getCfValid( $cf ) {
        if( preg_match("/^RAND_/", $cf ) || empty($cf) ) return null;

        $ana = \common\models\UtlAnagrafica::find()->where(['codfiscale'=>$cf])->one();
        if($ana) return $ana->codfiscale;
    ...
    private function objectMapping($data, $type = 'organizzazione') {
        ...
        switch($type) {
            case 'organizzazione':
                ...
                if ( $rappr_cf ) $arr['VolOrganizzazione']['cf_rappresentante_legale'] = $rappr_cf;
                if ( $ref_cf ) $arr['VolOrganizzazione']['cf_referente'] = $ref_cf;
            ...
            case 'ente':
                ...
                $rappr_cf = $this->getCfValid( @$data['rappresentanteLegale']['cf'] );
                $ref_cf = $this->getCfValid ( @$data['referente']['cf'] );
                if ( $rappr_cf ) $arr['EntEnte']['cf_rappresentante_legale'] = $rappr_cf;
                if ( $ref_cf ) $arr['EntEnte']['cf_referente'] = $ref_cf;
            ...
            case 'struttura':
                ...
                $rappr_cf = $this->getCfValid( @$data['rappresentanteLegale']['cf'] );
                $ref_cf = $this->getCfValid ( @$data['referente']['cf'] );
                if ( $rappr_cf ) $arr['StrStruttura']['cf_rappresentante_legale'] = $rappr_cf;
                if ( $ref_cf ) $arr['StrStruttura']['cf_referente'] = $ref_cf;
            ...
            case 'sede':
                ...
                if(!empty($email)) $arr['VolSede']['email'] = $email;
                if(!empty($email_pec)) $arr['VolSede']['email_pec'] = $email_pec;
                if(!empty($telefono)) $arr['VolSede']['telefono'] = $telefono;
                if(!empty($altro_telefono)) $arr['VolSede']['altro_telefono'] = $altro_telefono;
                if(!empty($fax)) $arr['VolSede']['fax'] = $fax;
                if(!empty($altro_fax)) $arr['VolSede']['altro_fax'] = $altro_fax;

                if(!empty($data['id_organizzazione'])) $arr['VolSede']['id_organizzazione'] = $data['id_organizzazione'];
            ...
            case 'automezzo':
                ...
                foreach ($f as $key) {
                    if($this->getMetaRisorsaValue($data, $key)) $data_[$key] = $this->getMetaRisorsaValue($data, $key);
                }

                $data_['idorganizzazione'] = @$data['idorganizzazione'];
                $data_['idsede'] = @$data['idsede'];
                if(!empty($data['idtipo'])) $data_['idtipo'] = $data['idtipo'];
            ...
            case 'volontario':
                ...
                if(!empty($data['id_sede'])) $data_['VolVolontario']['id_sede'] = $data['id_sede'];
                
                if(!empty($data['id_organizzazione'])) $data_['VolVolontario']['id_organizzazione'] = $data['id_organizzazione'];
                
                if(!empty($data['id_anagrafica'])) $data_['VolVolontario']['id_anagrafica'] = $data['id_anagrafica'];
    ...
    private function org_update() {
        ...
        if(!$org) $org = new \common\models\VolOrganizzazione;
        ...
        if(!empty($this->message['data']['rappresentanteLegale'])) $this->syncAnagrafica($this->message['data']['rappresentanteLegale']);
        if(!empty($this->message['data']['referente'])) $this->syncAnagrafica($this->message['data']['referente']);
    ...
    private function ente_update() {
        ...
        if(!$org) $org = new \common\models\ente\EntEnte;
    ...
    private function struttura_update() {
        ...
        if(!$org) $org = new \common\models\struttura\StrStruttura;
    ...
    private function update_automezzo()
    {
        ...
        if(!$type) $type = new \common\models\UtlAutomezzoTipo;
        ...
        if(!$automezzo) $automezzo = new \common\models\UtlAutomezzo;
        ...
        if(!empty($org)) {
            ...
            if(!empty($sede)) : $this->message['data']['idsede'] = $sede->id; endif;
            $this->message['data']['idorganizzazione'] = $org->id;

        } else {
            if(!empty($this->message['data']['organizzazione']['num_elenco_territoriale'])) return;
    ...
    private function update_attrezzatura()
    {
        ...
        if(!$type) $type = new \common\models\UtlAttrezzaturaTipo;
        ...
        if(!$attrezzatura) $attrezzatura = new \common\models\UtlAttrezzatura;
        ...
        $attrezzatura->load( $this->objectMapping( $this->message['data'], 'attrezzatura'));
    ...
    private function ente_sede_update()
    {
        ...
        if(!$sede) $sede = new \common\models\ente\EntEnteSede();
    ...
    private function struttura_sede_update()
    {
        ...
        if(!$sede) $sede = new \common\models\struttura\StrStrutturaSede();
    ...
    public function updated_anagraficavolontario() {
        ...
        $ids = [];
        foreach ($ana as $a) $ids[] = $a->id;

        if(count($ids) <= 0) return;

        Yii::$app->db->createCommand()
            ->update( \common\models\VolVolontario::tableName(), 
    ...
    private function volontario_update() 
    {
        ...
        if(!$ana) {
            ...
            if(!$vol) $vol = new \common\models\VolVolontario;
    ...
    private function tipoRisorsa_update()
    {
         ...
         * In base a mappatura scegliamo se aggiornare un automezzo o una attrezzatura
         */
        if(empty($this->message['data']['categoria'])) return;
        ...
        if(!$tipo_risorsa) $tipo_risorsa = ($type == 'automezzo') ? new \common\models\UtlAutomezzoTipo : new \common\models\UtlAttrezzaturaTipo;
    ...
    public function deleted_tipoRisorsa()
    {
        ...
        $tipo_risorsa = $this->getTipoRisorsa($this->message['data']['id'],$this->message['data']['descrizione'], $type);
        if($tipo_risorsa) $tipo_risorsa->delete();
    }
    ...
    {   
        ...
        if(!$tipo_risorsa_meta) $tipo_risorsa_meta = new \common\models\tabelle\TblTipoRisorsaMeta;
    ...
    {
        $tipo_risorsa_meta = $this->getTipoRisorsaMeta($this->message['data']['id'], $this->message['data']['key']);
        if($tipo_risorsa_meta) $tipo_risorsa_meta->delete();
    }
    ...
    public function deleted_contatto()
    {
        ...
        if($c) {
            if(!$c->delete()) Yii::error("Impossibile eliminare contatto ".$c->id);
        }
    }
    ...
    public function deleted_indirizzo()
    {
        ...
        if($i) {
            if(!$i->delete()) Yii::error("Impossibile eliminare contatto ".$i->id);
        }
    }
    ...
    private function specializzazione_updated()
    {
        ...
        if(!$t) $t = new \common\models\UtlSpecializzazione;
    ...
    private function sezioneSpecialistica_updated()
    {
        ...
        if(!$t) $t = new \common\models\TblSezioneSpecialistica;
    ...
    private function ruoloVolontario_updated()
    {
        ...
        if(!$t) $t = new \common\models\TblRuoloVolontario;
