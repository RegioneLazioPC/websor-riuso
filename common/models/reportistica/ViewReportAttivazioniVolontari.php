 ...
 */
class ViewReportAttivazioniVolontari extends \yii\db\ActiveRecord
{
    public $data_dal, $data_al, $aggregatore;
    public static function tableName()
    {
        return 'view_report_attivazioni_volontari';
    }
    ...
    public function search($params)
    {
        ...
        $query = ViewReportAttivazioniVolontari::find();
        ...
        if(!empty($this->cognome)){
            $query->andFilterWhere([ 'ilike', 'cognome', $this->cognome ]);
        }

        $dataProvider->sort->attributes['cognome'] = [
            'asc' => ['cognome' => SORT_ASC],
            'desc' => ['cognome' => SORT_DESC],
        ];

        if(!empty($this->codfiscale)){
            $query->andFilterWhere([ 'codfiscale' => $this->codfiscale ]);
        }

        $dataProvider->sort->attributes['codfiscale'] = [
            'asc' => ['codfiscale' => SORT_ASC],
            'desc' => ['codfiscale' => SORT_DESC],
        ];
