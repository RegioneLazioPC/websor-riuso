...
/**
 * ViewEntiTaskEventoSearch represents the model behind the search form about `common\models\ViewEntiTaskEvento`.
 */
class ViewEntiTaskEventoSearch extends ViewEntiTaskEvento
{
    ...
    public function search($params)
    {
        ...
        $query = ViewEntiTaskEvento::find()->where("stato != 'Chiuso'")
    ...
    {
        // di base è l'id del task dalla route
        $id_task = Yii::$app->request->get('id');
        
        $query = ViewEntiTaskEvento::find()->where(['id_utl_task' => $id_task]);
    ...
    {
        $query = ViewEntiTaskEvento::find()->where(['stato' => 'Chiuso'])->andWhere(['archived'=>0])->joinWith(['comune','comune.provincia'])->groupBy('utl_evento.id');
    ...
    {
        $query = ViewEntiTaskEvento::find()->where(['stato' => 'Chiuso'])->andWhere(['archived'=>1])->joinWith(['comune','comune.provincia'])->groupBy('utl_evento.id');
