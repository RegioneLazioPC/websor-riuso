...
/**
 * Class m190624_141210_alter_view_for_sezioni_specialistiche
 */
class m190624_141210_alter_view_for_sezioni_specialistiche extends Migration
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        Yii::$app->db
            ->createCommand("DROP MATERIALIZED VIEW IF EXISTS routing.m_view_organizzazioni")
            ->execute();
        ...
        Yii::$app->db->createCommand("
            CREATE VIEW view_organizzazioni AS
        ...
        Yii::$app->db->createCommand("CREATE MATERIALIZED VIEW routing.m_view_organizzazioni AS 
             SELECT *
               FROM routing.view_organizzazioni;");
    }
    ...
    public function safeDown()
    {
        ...
        Yii::$app->db->createCommand("DROP VIEW view_organizzazioni")
            ->execute();
        Yii::$app->db->createCommand("
            CREATE OR REPLACE VIEW view_organizzazioni AS
            SELECT utl_automezzo.id AS ref_id,
            utl_automezzo.targa AS ref_identifier,
            'automezzo'::text AS tipologia_risorsa,
            utl_automezzo.engaged AS ref_engaged,
            ...
            vol_organizzazione.email_responsabile,
            vol_organizzazione.nome_responsabile,
            string_agg(concat(org_contatto.note, ' - ', org_contatto.contatto), ', '::text) AS contatti_attivazioni
           FROM ((((((((((utl_automezzo
             LEFT JOIN vol_sede ON ((vol_sede.id = utl_automezzo.idsede)))
             LEFT JOIN utl_specializzazione ON ((utl_specializzazione.id = vol_sede.id_specializzazione)))
             LEFT JOIN vol_organizzazione ON ((vol_organizzazione.id = vol_sede.id_organizzazione)))
        ...
        UNION
         SELECT vol_sede.id AS ref_id,
           ...
           FROM ((((((vol_sede
             ...
             LEFT JOIN loc_provincia ON ((loc_provincia.id = loc_comune.id_provincia)))
          WHERE (vol_organizzazione.stato_iscrizione = 3)
          GROUP BY vol_sede.id, utl_specializzazione.descrizione, utl_specializzazione.id, vol_organizzazione.id, loc_comune.id, loc_provincia.id;
          ")->execute();
    ...
    {
        echo "m190624_141210_alter_view_for_sezioni_specialistiche cannot be reverted.\n";
